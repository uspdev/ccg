...
@section('title', config('app.name') . ' - Currículo ' . $curriculo['id'] . ' - Disciplinas Obrigatórias Equivalentes -
  ' . $disciplinaObrigatoria[0]['coddis'])

@section('content')

  @include('flash')

  <h3>Curriculo {{ $curriculo['id'] }} - Editar Disciplina Obrigatória Equivalente -
    {{ $disciplinaObrigatoria[0]['coddis'] }}</h3>
              ...
              <th>
                <div class="form-group">
                  ...
                  <input type="hidden" name="coddis" id="coddis"
                    value="{{ $disciplinaObrigatoriaEquivalente[0]['coddis'] }}">
                </div>
              </th>
            </tr>
          </thead>
