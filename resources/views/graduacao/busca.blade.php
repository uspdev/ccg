          		...
          		<div class="box-primary">
            		<div class="box-header">
                        ...
                        <img style="margin-left: 0px; margin-bottom: 10px;" class="profile-user-img img-responsive img-circle" 
                            src="data: image/jpeg; base64, {{ Uspdev\Wsfoto::obter($alunoGraduacao['codpes']) }}" alt="{{ $alunoGraduacao['nompes'] }}" />
              			@endif
              			
              			<h3 class="box-title">{{ $alunoGraduacao['codpes'] }} - {{ $alunoGraduacao['nompes'] }}</h3>
            		</div>
            		<div class="box-body table-responsive no-padding">
              			<table class="table table-hover">
                			<tr>
                  				<th>Curso</th>
                  				<td>{{ $alunoGraduacao['codcur'] }} - {{ $alunoGraduacao['nomcur'] }}</td>
                			</tr>
                			<tr>
                  				<th>Habilitação</td>
                  				<td>{{ $alunoGraduacao['codhab'] }} - {{ $alunoGraduacao['nomhab'] }}</td>
                			</tr>
                			<tr>
                  				<th>Ano de ingresso</td>
                  				<td>{{ Carbon\Carbon::parse($alunoGraduacao['dtainivin'])->format('Y') }}</td>
                			</tr>
                			<tr>
                  				<th>Programa</td>
                  				<td>{{ $alunoGraduacao['codpgm'] }}</td>
                			</tr>
              			</table>
            		</div>
          		</div>
